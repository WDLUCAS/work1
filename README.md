# HomeWork
软件工程作业<br/>
HomeWork1 实现以下功能：（1）link到其他主页（2）显示本地时间（3）点击按钮，实现页面跳转<br/>
# HomeWork1
进入Hello页面，显示服务器时间和国科大官网的链接<br/>
![image](./image/index.png)<br/>
点击UCAS后跳转到中国科学院大学的官网，如图所示：<br/>
![image](./image/UCAS.png)<br/>
在Hello页面点击database跳转到数据库操作页面，该页面显示数据库记录并提供修改数据库功能，如图所示：<br/>
![image](./image/database.png)<br/>
点击New High score跳转到添加分数页面，如图所示：<br/>
![image](./image/addRecoder.png)<br/>
添加完数据后跳转到数据库显示页面，如图所示：<br/>
![image](./image/database2.png)<br/>
